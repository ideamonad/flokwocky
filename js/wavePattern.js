...
module.exports = function (Gibber) {
  ...
  const WavePattern = {
    ...
    create(abstractGraph, values, mode = null) {
      ...
      const patternOutputFnc = function (isViz = true) {
        ...
        if (typeof pattern.genReplace === 'function') 
        { 
          pattern.genReplace(output) 
        }
        ...
        if (pattern.running === false) {
          Gibber.Environment.animationScheduler.add(
            pattern.runVisualization, 1000 / 60)
      ...
      if (abstractGraph.paramID === undefined)
        abstractGraph.paramID = pattern.paramID
