...
let Environment = {
  ...
  clearVisualization() {
    if (Gibber !== null) {
      console.log("clear all code visualization");

      this.animationScheduler.clear()

      for (let i = 0; i < Gibber.Seq._seqs.length; i++) {
        Gibber.Seq._seqs[i].clearVisualization()
      }

      Gibber.Gen.clearAllVisualization()
  ...
  evalCode(
    ...
    const { immediate, refresh, bindTrack } = option

    // 创建执行函数
    const logic = bindTrack ?
      new Function(code).bind(Gibber.currentTrack) :
      new Function(code)
    const theGibber = Gibber;
    ...
    const isConnected =
      Gibber.Communication.connected.live ||
    ...
    if (immediate || !isConnected) {
      ...
      // 对齐节奏执行
      Gibber.Scheduler.functionsToExecute.push(play)
      return {
        executed: false,
      }
    }
